...
export default function Delete() {
  ...
  const fetchAllStarships = async () => {
    ...
    try {
      ...
      const response = await fetch(url, {
        headers: { "Content-Type": "application/json", "X-Domain": domain ,"Authorization": `Bearer ${token}`},
      });
  ...
  const confirmDelete = (id, name) => {
    setDeleteAlert({ show: true, id, name });
  };

  const handleDeleteConfirm = async () => {
    if (!domain || !deleteAlert.id) {
      setError("API domain not set or invalid ID.");
    ...
    setLoading(true);
    try {
      const url = `${domain}/api/Starships?id=${deleteAlert.id}`;
      const response = await fetch(url, {
        method: "DELETE",
        headers: { "Content-Type": "application/json", "X-Domain": domain ,"Authorization": `Bearer ${token}`},
      });

      if (!response.ok) throw new Error("Failed to delete starship");

      setAlertProps({ type: "success", message: `"${deleteAlert.name}" deleted successfully!` });
      fetchAllStarships();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const handleSingleSearchChange = (e) => setSingleSearchTerm(e.target.value);
          ...
          <tbody>
            {loading ? (
              ...
              paginatedData.map((row, idx) => (
                ...
                >
                  <td className="p-3 border-b border-white/10 whitespace-nowrap flex space-x-2">
                    <button
                      onClick={() => confirmDelete(row.id, row.name)}
                      className="bg-red-600 hover:bg-red-800 transition px-3 py-1 rounded-md text-sm"
                    >
                      Delete
                    </button>
                  </td>
